...
$('.card').click(function(){
    let employeePic = $(this).find('.card-img').attr('src');
    let employeeName = $(this).find('#name').text(); //name
    let employeeEmail = $(this).find('.card-text').eq(0).text(); //email
    let employeeCity = $(this).find('.card-text.cap').text(); //location
    let employeeCell = $(this).find('.hidden-info').eq(0).text(); //cell
    let employeeAddress = $(this).find('.hidden-info').eq(1).text(); //address
    let dateLongVersion = new Date($(this).find('.hidden-info').eq(2).text()); //dob
    let month = dateLongVersion.getMonth();
    let date = dateLongVersion.getDate();
    let year = dateLongVersion.getFullYear().toString().slice(2);
    //let employeeBirthday = dateLongVersion.toDateString().slice(4);
    console.log(this)
    //console.log(pic)
// });
// };
...
//$('hr').after(`<p class="modal-text">Birthday: ${employeeBirthday}</p>`)
$('hr').after(`<p class="modal-text">Birthday: ${month}/${date}/${year}</p>`)
.after(`<p class="modal-text">${employeeAddress}</p>`)
.after(`<p class="modal-text">${employeeCell}</p>`);
